import { resolveFromScripts, upsertRaces, upsertRiders, loadSeedFile } from "./seed-utils";
...
async function main() {
  ...
  const riders = await loadSeedFile(ridersFile);
  await upsertRiders(riders);

  console.log(`Seeding races from ${racesFile}`);
  const races = await loadSeedFile(racesFile);
